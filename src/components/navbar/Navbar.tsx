...
const Navbar = () => {
  ...
  return (
    ...
    <nav
      className={`${isSettingsMenuOpen ? "sm:bg-slate-200/80 sm:shadow-lg dark:sm:bg-slate-800/80 dark:sm:shadow-none" : "sm:bg-slate-100/80 dark:sm:bg-slate-900/80"} relative flex h-18 w-full items-center sm:fixed sm:top-0 sm:z-50 sm:h-20 sm:justify-between sm:px-12 sm:backdrop-blur-md sm:transition-[background-color_shadow] md:px-24`}
    >
      <NavbarLogo />

      {/* This is its own bar at the bottom of the screen on mobile */}
      <div
        className={`${isSettingsMenuOpen ? "max-sm:bg-slate-200/80 dark:max-sm:bg-slate-800/80" : "max-sm:bg-slate-100/80 dark:max-sm:bg-slate-900/80"} flex items-center gap-2 max-sm:fixed max-sm:bottom-0 max-sm:left-0 max-sm:z-50 max-sm:h-16 max-sm:w-full max-sm:justify-center max-sm:backdrop-blur-md max-sm:transition-[background-color]`}
      >
        <NavbarLinks />
        <SettingsMenuButton
